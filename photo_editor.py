...
def process_image(image, flip, rotate, bw, resize, flip_type, rotate_degree, resize_percentage, brightness, contrast, hue, saturation, remove_bg, format):
    try:
        # Debugging prints
        ...
        print(f"Brightness: {brightness}, Contrast: {contrast}, Hue: {hue}, Saturation: {saturation}, Remove BG: {remove_bg}")
        ...
        # Adjust hue (convert image to HSV, modify hue, then convert back to RGB)
        if hue != 1.0:
            image = image.convert("HSV")
            np_image = np.array(image)
            np_image[..., 0] = (np_image[..., 0] * hue) % 256  # Hue value must be within 0-255
...
iface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(0.1, 2.0, label="Hue", value=1.0, step=0.1),
